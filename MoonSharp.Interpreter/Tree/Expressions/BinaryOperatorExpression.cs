...
namespace MoonSharp.Interpreter.Tree.Expressions
{
	...
	class BinaryOperatorExpression : Expression
	{
		...
		private FixedNumber EvalArithmetic(DynValue v1, DynValue v2)
		{
            FixedNumber? nd1 = v1.CastToNumber();
            FixedNumber? nd2 = v2.CastToNumber();

			if (nd1 == null || nd2 == null)
				throw new DynamicExpressionException("Attempt to perform arithmetic on non-numbers.");

            FixedNumber d1 = nd1.Value;
            FixedNumber d2 = nd2.Value;
			...
			switch (m_Operator)
			{
				...
				case Operator.Mod:
					{
                        FixedNumber mod = FixedNumber.IEEERemainder(d1, d2);
						if (mod < 0) mod += d2;
						return mod;
					}
				default:
