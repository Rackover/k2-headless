...
namespace MoonSharp.Interpreter
{
	...
	public sealed class DynValue
	{
		...
		/// </summary>
		public FixedNumber Number { get { return m_Number; } }
		/// <summary>
		...
		/// </summary>
		public static DynValue NewNumber(FixedNumber num)
		{
			return new DynValue()
			{
				m_Number = num,
		...
		public FixedNumber? CastToNumber()
		{
			DynValue rv = ToScalar();
			if (rv.Type == DataType.Number)
			{
			...
			{
				FixedNumber num;
				if (FixedNumber.TryParse(rv.String, NumberStyles.Any, CultureInfo.InvariantCulture, out num))
					return num;
			}
			return null;
		}
		...
		/// </summary>
		internal void AssignNumber(FixedNumber num)
		{
			if (this.ReadOnly)
		...
		public DynValue CheckType(string funcName, DataType desiredType, int argNum = -1, TypeValidationFlags flags = TypeValidationFlags.Default)
		{
			...
			if (autoConvert)
			{
				...
				{
                    FixedNumber? v = this.CastToNumber();
					if (v.HasValue)
