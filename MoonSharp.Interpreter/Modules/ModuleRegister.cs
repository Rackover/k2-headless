...
namespace MoonSharp.Interpreter
{
	...
	public static class ModuleRegister
	{
		...
		private static void RegisterScriptFieldAsConst(FieldInfo fi, object o, Table table, Type t, string name)
		{
			...
			else if (fi.FieldType == typeof(FixedNumber))
			{
                FixedNumber val = (FixedNumber)fi.GetValue(o);
				table.Set(name, DynValue.NewNumber(val));
			}
			else
			{
				throw new ArgumentException(string.Format("Field {0} does not have the right type - it must be string or FixedNumber.", name));
			}
		}
